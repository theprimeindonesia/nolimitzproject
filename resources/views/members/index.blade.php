...
<div class="page-content">
    <div class="panel panel-bordered">
        <div class="panel-heading">
            <h3 class="panel-title">Members Management</h3>
        </div>
        <div class="panel-body">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-15">
                        <a href="{{ route('members.create') }}" class="btn btn-outline btn-primary">
                        <i class="icon wb-plus" aria-hidden="true"></i> Add New Member
                        </a>
                    </div>
                </div>
            </div>
            ...
            <table class="table table-hover dataTable table-striped w-full" data-plugin="dataTable">
                ...
                @foreach($data as $x)
                    <tr>
                        ...
                        <td>{{$x['dompet']}}</td>
                        <td>
                            <a href="{{route('members.detail',$x->members_id)}}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"><i class="icon wb-eye" aria-hidden="true"></i></a>
                            <a href="{{route('members.edit',$x->members_id)}}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"><i class="icon wb-pencil" aria-hidden="true"></i></a>
                            <form id="remove-user" action="{{route('members.destroy',$x->members_id)}}" method="POST" style="display: inline-block;">
                                {{method_field('DELETE')}}
                                @csrf
