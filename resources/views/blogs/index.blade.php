...
<div class="page-content">
    <div class="panel panel-bordered">
        <div class="panel-heading">
            <h3 class="panel-title">Blogs Management</h3>
        </div>
        <div class="panel-body">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-15">
                        <a href="{{ route('blogs.create') }}" class="btn btn-outline btn-primary">
                        <i class="icon wb-plus" aria-hidden="true"></i> Add New Blog
                        </a>
                    </div>
                </div>
            </div>
            ...
            <table class="table table-hover dataTable table-striped w-full" data-plugin="dataTable">
                ...
                <tr>
                	<td>{{ $i++ }}</td>
                	<td>{{ $row->categoryBlogs->name_ind }}</td>
                	<td>{{ $row->title_ind }}</td>
                	<td>{{ str_limit(strip_tags($row->article_ind), 100) }}</td>
                	<td>
                		<a href="{{ route('blogs.edit', $row->blogs_id) }}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"><i class="icon wb-edit" aria-hidden="true"></i></a>
                		<form id="remove-user" action="{{ route('blogs.destroy', $row->blogs_id) }}" method="POST" style="display: inline-block;">
                			{{method_field('DELETE')}}
                			@csrf
                ...
                {{-- @php($i = 1)
                    ...
                    <tr>
                        ...
                        <td>{{ $x->name_en }}</td>
                        <td>
                            <a href="{{ route('category-blogs.edit', $x->category_blogs_id) }}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"><i class="icon wb-edit" aria-hidden="true"></i></a>
                            <form id="remove-user" action="{{ route('category-blogs.destroy', $x->category_blogs_id) }}" method="POST" style="display: inline-block;">
                                {{method_field('DELETE')}}
                                @csrf
                                <button onclick="return confirm('Are you sure you want to delete this ?');" type="submit" class="btn btn-sm btn-icon btn-pure btn-default on-default remove-row"><i class="icon wb-trash" aria-hidden="true"></i></button>
                            </form>
