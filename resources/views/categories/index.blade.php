...
<div class="page-content">
    <div class="panel panel-bordered">
        <div class="panel-heading">
            <h3 class="panel-title">Product Categories Management</h3>
        </div>
        <div class="panel-body">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-15">
                        <a href="{{ route('categories.create') }}" class="btn btn-outline btn-primary">
                        <i class="icon wb-plus" aria-hidden="true"></i> Add New Categories
                        </a>
                    </div>
                </div>
            </div>
            ...
            <table class="table table-hover dataTable table-striped w-full" data-plugin="dataTable">
                ...
                @php($i = 1)
                    ...
                    <tr>
                        <td>{{ $i++ }}</td>
                        <td><img src="../images/categories/{{ $x->images }}" width='100px'></td>
                        <td>{{ $x->name_ind }}</td>
                        <td>{{ $x->name_en }}</td>
                        <td>
                            <a href="{{ route('categories.edit', $x->categories_id) }}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"><i class="icon wb-edit" aria-hidden="true"></i></a>
                            <form id="remove-user" action="{{ route('categories.destroy', $x->categories_id) }}" method="POST" style="display: inline-block;">
                                {{method_field('DELETE')}}
                                @csrf
