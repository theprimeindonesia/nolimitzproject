            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-15">
                        ...
                        <i class="icon wb-plus" aria-hidden="true"></i> Add New Role
                        </a>
                    </div>
                </div>
            </div>
            ...
            <table class="table table-hover dataTable table-striped w-full" data-plugin="dataTable">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @can('role-edit')
                            <a href="{{ route('roles.edit',$role->id) }}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"><i class="icon wb-edit" aria-hidden="true"></i></a>
                            @endcan
                            @can('role-delete')
                            <form id="remove-user" action="{{ route('roles.destroy', $role->id) }}" method="POST" style="display: inline-block;">
                                {{method_field('DELETE')}}
                                @csrf
                                <button onclick="return confirm('Are you sure you want to delete this role?');" type="submit" class="btn btn-sm btn-icon btn-pure btn-default on-default remove-row"><i class="icon wb-trash" aria-hidden="true"></i></button>
                            </form>
                            @endcan
                        </td>
                    </tr>
